...
if( isset($_POST['bp']) ){	
	header('Content-Disposition: attachment; filename="./download/Burp_Suite_Pro_v1.7.32_Loader_Keygen.zip"'); //设置MIME用户作为附件	
	header('Content-Length: '.filesize('./download/Burp_Suite_Pro_v1.7.32_Loader_Keygen.zip')); //设置内容长度
exit;
}elseif(isset($_POST['caidao'])){
	header('Content-Disposition: attachment; filename="/download/caidao-20160622.zip"  ');
	header('Content-Length: '.filesize('/download/caidao-20160622.zip')); 
...
exit;
	...
	header('Content-Disposition: attachment; filename="./download/RST.zip" ');
	header('Content-Length: '.filesize('./download/RST.zip')); 
exit;	
}else{
