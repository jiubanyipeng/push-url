...
sub DownContent(intTitleLen)
	...
	do while not rsDown.eof
		...
		strTemp=strTemp & "<table width=98% border=0 cellSpacing=1 cellpadding=3 bgColor=#b8b8b8>"
			...
			strTemp=strTemp & "<FONT COLOR=#0000FF>整理日期：</FONT>" & FormatDateTime(rsDown("AddDate"),2) & "&nbsp;&nbsp;"
			strTemp=strTemp & "<FONT COLOR=#0000FF>文件大小：</FONT>" & rsDown("FileSize") & "</td>"
			...
			strTemp=strTemp & "<a href=" & rsDown("DownloadUrl") & " target=_blank><FONT COLOR=#0000FF>下载</FONT></a> | "
			strTemp=strTemp & "<a href=DownloadShow.asp?ID=" & rsDown("ID") & "><FONT COLOR=#0000FF>查看</FONT></a>&nbsp;"
			...
			strTemp=strTemp & "<FONT COLOR=#0000FF>软件语言：</FONT>" & rsDown("Language") & "&nbsp;&nbsp;"
			strTemp=strTemp & "<FONT COLOR=#0000FF>运行环境：</FONT>" & rsDown("System") & ""
			strTemp=strTemp & "</td></tr>"
			strTemp=strTemp & ""
