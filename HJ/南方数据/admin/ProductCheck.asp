...
%>
        <table class="border" border="0" cellspacing="2" width="620" cellpadding="0" style="word-break:break-all">
          ...
          <tr class="tdbg"> 
            ...
            <td width="37" align="center" bgcolor="#ECF5FF"><%=rs("articleid")%></td>
            <td bgcolor="#ECF5FF"> <a href="../ProductShow.asp?ArticleID=<%=rs("articleid")%>" title="<%=replace(left(nohtml(rs("Content")),200),chr(34),"") & "……"%>">&nbsp;<%=rs("title")%></a></td>
	  ...
	  if session("purview")<=2 or PurviewChecked=True then
	  	If rs("Passed")=False Then %> <a href="ProductCheckSet.asp?Action=Check&ArticleID=<%=rs("ArticleID")%>">通过审核</a> 
              <% Else %> <a href="ProductCheckSet.asp?Action=CancelCheck&ArticleID=<%=rs("ArticleID")%>">取消通过</a> 
              <%
	    End If
	  end if %> </td>
          </tr>
